				...
				<div id="collapseTwo" class="collapse show"
					...
					<div class="bg-white py-2 collapse-inner rounded">
						...
						<a class="collapse-item" href="/admin/hotel">Khách sạn</a> <a
							...
							class="collapse-item active" href="/admin/roomtype">Loại
							phòng</a>
					</div>
				</div></li>
			...
			<div id="content">
				...
				<div class="ps-3 pe-3">
					<div class="w-100"
							...
							<div class="container-fluid">
								...
								<form class="d-flex" role="search" style="margin-right: 50px;">
									<input class="form-control me-2" style="width: 400px"
										type="search" placeholder="Nhập tên loại phòng"
										aria-label="Search" value="${keyword}" name="keyword">
							...
							<table class="table table-striped" style="height: 278px;">
								...
								<tbody>
									<c:forEach var="roomtype" items="${roomTypes}">
										<tr>
											<th scope="row">${roomtype.typeid}</th>
											<td>${roomtype.typeName}</td>
											<td><fmt:formatNumber type="currency"
													value="${roomtype.price}" pattern="#,##0.00 vnđ"
													maxFractionDigits="0" /></td>
											<td>${roomtype.capacity}</td>
											<td><a type="button" class="btn btn-primary btn-sm"
												href="/admin/roomtype/edit/${roomtype.typeid}">Edit</a>
										</tr>
									</c:forEach>
								</tbody>
							</table>
								...
								<ul class="pagination">
									<c:if test="${totalPages > 1}">
										...
										<li class="page-item"><a class="page-link"
											href="/admin/roomtype?page=0&keyword=${keyword}"
										...
										<c:forEach
											...
											<c:if test="${i < totalPages}">
												<li class="page-item ${currentPage == i ? 'active' : ''}">
													<a class="page-link"
													href="/admin/roomtype?page=${i}&keyword=${keyword}">${i + 1}</a>
												</li>
											</c:if>
										</c:forEach>
										...
										<!-- Trang cuối cùng -->
										<li class="page-item"><a class="page-link"
											href="/admin/roomtype?page=${totalPages - 1}&keyword=${keyword}">Last</a></li>
									</c:if>
								</ul>
							</div>
						</div>
						<div class="row">
							<div class="col">
								<a class="btn btn-danger float-end me-4"
									href="/admin/roomtype-form">Thêm mới</a> <a
									href="/admin/roomtype"
									class="btn btn-outline-light float-end me-2">Reset</a>
							</div>
						</div>
					</div>
				</div>
